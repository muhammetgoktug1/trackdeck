...
router.get('/', async (req, res) => {
  ...
  const [data, total] = await Promise.all([
    NoteCategory.find().sort({ name: 1 }).skip((page - 1) * limit).limit(limit),
    NoteCategory.countDocuments(),
  ]);
...
// Kategoriyi sil (bağlı notlardan ilişkiyi kaldır)
router.delete('/:id', async (req, res) => {
  const id = parseId(req);
  if (!id) return res.status(400).json({ message: 'Geçersiz kimlik' });

  const category = await NoteCategory.findByIdAndDelete(id);
  if (!category) return res.status(404).json({ message: 'Kategori bulunamadı' });

  await Note.updateMany({ category: id }, { $unset: { category: '' } });
