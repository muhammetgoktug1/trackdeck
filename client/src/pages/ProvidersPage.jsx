    ...
    <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/50">
      <div className="flex items-center justify-between px-5 py-4">
        <div className="flex items-center gap-2.5">
          <h2 className="text-sm font-semibold text-white">Sağlayıcılar</h2>
          {!loading && (
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center px-6 pb-14 pt-10 text-center">
          ...
          <h3 className="mt-4 text-[15px] font-semibold text-zinc-200">
            Henüz sağlayıcı tanımı yok
          </h3>
          <p className="mt-1 max-w-sm text-sm leading-relaxed text-zinc-500">
            Firmaları bir kez burada tanımla; domain ve sunucu eklerken listeden
            seç.
          </p>
          <button
            type="button"
            ...
            <table className="w-full min-w-[560px] text-left text-sm">
              <thead>
                <tr className="border-t border-zinc-800/80 text-[11px] font-semibold uppercase tracking-wider text-zinc-600">
                  <th className="px-5 py-3">Sağlayıcı</th>
                ...
                {list.data.map((p) => (
                  <tr key={p.id} className="transition-colors hover:bg-zinc-800/30">
                    <td className="max-w-[420px] px-5 py-4">
                      <div className="flex items-center gap-2.5">
                        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-zinc-800 ring-1 ring-zinc-700/60">
