...
import { formatDateTR, expiryClass } from '../lib/format.js';
    ...
    <div className="overflow-hidden rounded-2xl border border-zinc-800/80 bg-zinc-900/50">
      <div className="flex items-center justify-between px-5 py-4">
        <div className="flex items-center gap-2.5">
          <h2 className="text-sm font-semibold text-white">Domainler</h2>
          {!loading && (
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center px-6 pb-14 pt-10 text-center">
          ...
          <p className="mt-1 max-w-sm text-sm leading-relaxed text-zinc-500">
            Domainlerini ekleyerek bitiş tarihlerini takibe al; monitörlere de
            bağlayabilirsin.
          </p>
          <button
            type="button"
            ...
            <table className="w-full min-w-[820px] text-left text-sm">
              <thead>
                <tr className="border-t border-zinc-800/80 text-[11px] font-semibold uppercase tracking-wider text-zinc-600">
                  ...
                  <th className="px-4 py-3">Kayıt Firması</th>
              ...
              <tbody className="divide-y divide-zinc-800/50">
                {list.data.map((d) => {
                  const expiry = expiryClass(d.expiresAt);
                  return (
                    <tr key={d.id} className="transition-colors hover:bg-zinc-800/30">
                      ...
                      <td className="px-4 py-4 text-zinc-400">
                        {d.provider?.name || <span className="text-zinc-600">—</span>}
                      </td>
                      <td className="px-4 py-4 text-zinc-400">
                        {d.purchasedAt ? (
                          formatDateTR(d.purchasedAt)
                        ) : (
                        ...
                        {d.expiresAt ? (
                          <div className="flex flex-col gap-1">
                            <span className="text-zinc-300">
                              {formatDateTR(d.expiresAt)}
                            </span>
                            {expiry.chip && (
                              <span
                                className={`inline-flex w-fit items-center rounded-full px-2 py-0.5 text-[11px] font-semibold ring-1 ${expiry.chip}`}
                              >
                                {expiry.text}
                              </span>
                            )}
