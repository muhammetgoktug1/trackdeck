...
export default function ProviderModal({ open, provider, saving, error, onSave, onClose }) {
  ...
  useEffect(() => {
    if (open) {
      setForm(
        provider
          ? { name: provider.name, notes: provider.notes ?? '' }
          : EMPTY_FORM
      );
    }
  }, [open, provider]);
      ...
      <div className="w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-900 shadow-2xl shadow-black/50 animate-pop-in">
        <div className="flex items-center justify-between border-b border-zinc-800 px-6 py-4">
          <h2 className="text-[15px] font-bold text-white">
            {provider ? 'Sağlayıcıyı Düzenle' : 'Yeni Sağlayıcı'}
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={submit} className="flex flex-col gap-4 px-6 py-5">
          <div>
            ...
            <input
              ...
              placeholder="Örn: Hetzner"
              required
              maxLength={120}
              autoFocus
            />
            <p className="mt-1.5 text-[11px] text-zinc-600">
              Domainlerin kayıt firması ve sunucuların sağlayıcısı buradan seçilir
            </p>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Hesap notları, yenileme bilgileri..."
              maxLength={2000}
            />
          </div>
          ...
          <div className="mt-1 flex items-center justify-end gap-2.5">
            ...
            >
              ...
              {provider ? 'Kaydet' : 'Sağlayıcıyı Ekle'}
            </button>
          </div>
        </form>
      </div>
